...
// Python keywords (highlighted in blue #569cd6)
...
// Python control flow keywords (highlighted in pink/magenta #C586C0)
const PYTHON_CONTROL_FLOW = new Set([
  "if",
  "elif",
  "else",
  "for",
...
// Python built-in functions (highlighted as function calls #dcdcaa)
...
// Python decorators color (same as function call #dcdcaa)
const COLORS_PYTHON = {
  decorator: "#dcdcaa",
};
...
function tokenizePython(code) {
  ...
  // Helper to check if identifier is PascalCase (likely a class)
  function isPascalCase(str) {
    return /^[A-Z][a-zA-Z0-9]*$/.test(str);
  }
  ...
  while (i < code.length) {
    ...
    if (
      ...
      let value = tripleQuote;
      i += 3;
      while (i < code.length && !code.startsWith(tripleQuote, i)) {
        if (code[i] === "\\" && i + 1 < code.length) {
          value += code[i++] + code[i++];
        } else {
          value += code[i++];
      ...
      if (code.startsWith(tripleQuote, i)) {
        value += tripleQuote;
        i += 3;
      }
      tokens.push({ type: "string", value });
    ...
    if (
      ...
      const fPrefix = code[i];
      i++;
      const quote = code[i];
      let value = fPrefix + quote;
      i++;
      ...
      if (i < code.length && code[i] === quote) value += code[i++];
      tokens.push({ type: "string", value });
      continue;
    }
    ...
    if (
      ...
      const rPrefix = code[i];
      i++;
      const quote = code[i];
      let value = rPrefix + quote;
      i++;
      while (i < code.length && code[i] !== quote && code[i] !== "\n") {
        value += code[i++];
      }
    ...
    if (
      ...
      const bPrefix = code[i];
      i++;
      const quote = code[i];
      let value = bPrefix + quote;
      i++;
      while (i < code.length && code[i] !== quote && code[i] !== "\n") {
        if (code[i] === "\\" && i + 1 < code.length) {
          value += code[i++] + code[i++];
    ...
    // Numbers (including floats, hex, binary, octal, complex)
    if (/[0-9]/.test(code[i])) {
      let value = code[i++];
      // Handle hex (0x), binary (0b), octal (0o)
      if (value === "0" && i < code.length && /[xXbBoO]/.test(code[i])) {
        value += code[i++];
        ...
        if (i < code.length && /[jJ]/.test(code[i])) {
          value += code[i++];
        }
      }
      tokens.push({ type: "number", value });
    ...
    if (/[a-zA-Z_]/.test(code[i])) {
      ...
      // Check keyword types
      if (PYTHON_CONTROL_FLOW.has(value)) {
        tokens.push({ type: "controlFlow", value });
      } else if (PYTHON_KEYWORDS.has(value)) {
        tokens.push({ type: "keyword", value });
      } else if (PYTHON_BUILTINS.has(value)) {
        // Check if followed by ( for function call styling
        let j = i;
        while (j < code.length && /\s/.test(code[j])) j++;
        if (code[j] === "(") {
          tokens.push({ type: "functionCall", value });
        ...
        // Check if followed by ( for function definition/call
        let j = i;
        while (j < code.length && /\s/.test(code[j])) j++;
        if (code[j] === "(") {
          tokens.push({ type: "functionCall", value });
